...
export async function POST(req: NextRequest) {
  ...
  const sql = getDb();
  try {
    const { name, type, url, data, category, department, subfolderId } = await req.json();
    const uploadedBy = session.user.name || session.user.email;
    
    const resource = await sql`
      INSERT INTO "LearningResource" ("name", "type", "url", "data", "category", "department", "subfolderId", "uploadedBy", "createdAt")
      VALUES (${name}, ${type}, ${url}, ${data}, ${category || 'Miscellaneous'}, ${department || 'General'}, ${subfolderId || null}, ${uploadedBy}, NOW())
      RETURNING *
    `;
