...
// POST /api/payments/tracker/[id]/share-advice
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { utrNumber, adviceRecipient, adviceCC, attachment } = await req.json();

    if (!adviceRecipient) {
      return NextResponse.json({ error: "Recipient email is required" }, { status: 400 });
    }

    // Update the record with latest advice details
    ...
    // Fetch vendor and payment info for the email
    const template = await sql`
      SELECT t.* FROM "PaymentTemplate" t
      WHERE t.id = ${occ.templateId}
    `;
    const vendorName = template[0]?.vendorName || "Vendor";
    const paymentDate = occ.actualDate || occ.dueDate;
    ...
    const subject = `Payment Advice_${vendorName}_${formatDate(paymentDate)}`;
        ...
        <div style="padding: 24px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 24px 0; background: #f8fafc; border-radius: 8px; overflow: hidden;">
            ...
            <tr>
              ...
              <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; font-weight: 700; color: #16a34a;">₹${Number(occ.amountPaid).toLocaleString('en-IN')}</td>
            </tr>
            <tr>
              <td style="padding: 12px; border-bottom: 1px solid #e2e8f0; font-weight: 600; color: #64748b;">Payment Date</td>
              <td style="padding: 12px; border-bottom: 1px solid #e2e8f0;">${formatDate(paymentDate)}</td>
            </tr>
            <tr>
    ...
    return NextResponse.json({ success: true, occ: result[0] });
  } catch (error: any) {
    console.error("Share advice error:", error);
