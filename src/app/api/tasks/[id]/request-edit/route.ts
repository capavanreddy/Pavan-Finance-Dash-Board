  ...
  try {
    ...
    const emailHtml = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 12px;">
        <h2 style="color: #2563eb; margin-top: 0;">Task Edit Request</h2>
        <p style="font-size: 16px; color: #334155;">User <strong>${userName}</strong> has requested to edit a completed task:</p>
        ...
        <div style="background: #f8fafc; padding: 20px; border-radius: 8px; margin: 20px 0; border: 1px solid #f1f5f9;">
          <table border="0" cellpadding="5" cellspacing="0" style="width: 100%; font-size: 14px;">
            <tr><td style="color: #64748b; width: 100px;">Task ID:</td><td style="color: #0f172a; font-weight: 600;">#${existingTask.id}</td></tr>
            <tr><td style="color: #64748b;">Task Name:</td><td style="color: #0f172a;">${existingTask.taskName}</td></tr>
            <tr><td style="color: #64748b;">Requested By:</td><td style="color: #0f172a;">${requestedBy}</td></tr>
          </table>
        </div>
    ...
    import("@/lib/email").then(({ sendEmail }) => {
      sendEmail({
        to: adminEmail,
        subject: `[Edit Request] #${existingTask.id} - ${existingTask.taskName}`,
        html: emailHtml
      });
    });
